...
- **Simple math (added 2026-09-26, at the owner's request, per `CLAUDE.md` §15).** A "Working the numbers" paragraph was inserted into step 2 (the everyday rule), between the sentence naming the everyday-rule light speed and the sentence about the tick duration. It walks through the same real-number diagonal-path arithmetic as Experiment 4 (the mirror separation, half the sideways distance, and the resulting diagonal and round-trip path lengths — all concrete numbers, no algebra), then divides that round-trip path by the unchanged tick duration to show, arithmetically, why the everyday rule's light speed exceeds `c`. This experiment's tutor already contained a daily-life example (the ball thrown from a moving train, required by the Introductory text section above) and substituted real numbers throughout its explanation, so no further addition was needed there. `everydayLightPath`, `everydaySidewaysDistancePerTick`, and `mirrorSeparation` were added to `Experiment5Tutor`'s props (already present on `InvariantLightSpeedResult`) to support this.
