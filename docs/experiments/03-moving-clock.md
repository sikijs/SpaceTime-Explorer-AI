...
- **Simple math (added 2026-09-26, at the owner's request, per `CLAUDE.md` §15).** After the "At {velocity}c..." summary, the non-baseline explanation adds three short paragraphs: "About the formula" (what `Δt_moving = Δt_lab × √(1 − v²)` means in plain words, and why `v` is written as a fraction of light speed), an explanation of the "shrinking factor" `√(1 − v²)` as a dial running from 1 (at `v = 0`, matching Experiment 2) down toward but never reaching 0 (as `v` approaches 1), and "Your numbers" (the learner's own `v`, `1 − v²`, and the square root worked out arithmetically, ending at the moving clock's actual reading). This shows the formula is self-consistent with real numbers but does not explain *why* the formula is true — that derivation (the light-clock geometry) stays reserved for Experiment 4, per this specification's "Key Design Decisions" above.
