...
Experiment 5 (why can't light go faster?) has an approved specification and is implemented, with its complete-flow test and final review done. It shows the light clock of Experiment 4 under two rules for light, an everyday rule in which speeds add and light's actual rule in which light always travels at `c` in the lab, so that the learner can see what the constancy of the speed of light is responsible for. Experiment 6 (does motion change length?) has an approved specification and is implemented, with its complete-flow test and final review done. It was chosen by the project owner beyond the planned progression, and it shows that time dilation with light at `c` requires a clock moving along its own length to be shorter, as seen from the lab (length contraction). Experiment 7 (relativity of simultaneity) has an approved specification (`docs/experiments/07-relativity-of-simultaneity.md`) and is implemented, with its complete-flow test, final review, and the owner's wording approval all done. It shows that a flash released from the center of the Experiment 6 rod reaches both ends at the same time in the rod's own frame, but not in the lab frame. Experiment 8 (drawing spacetime) has an approved specification (`docs/experiments/08-spacetime-diagrams.md`) and is implemented, with its complete-flow test and final review done; the owner's wording approval was skipped, not granted. It draws the Experiment 6/7 rod and flash as a spacetime diagram, in the lab frame only, without recomputing Experiment 7's results. Experiment 9 (same time, different line) has an approved specification (`docs/experiments/09-same-time-different-line.md`) and is implemented, with its complete-flow test, final review, and the owner's wording approval all done. It adds one line to the Experiment 8 diagram — the rod's own "same moment" line — through the same two events Experiment 7 and 8 already established, with no new physics. Experiment 10 (the twin paradox) has an approved specification (`docs/experiments/10-twin-paradox.md`) and is implemented, with its complete-flow test and final review done; the owner's wording approval has not yet been done. It applies Experiment 3's time dilation to a round-trip journey, resolving the classic twin paradox by showing that only the traveling twin turns around, breaking the symmetry between the two twins.
