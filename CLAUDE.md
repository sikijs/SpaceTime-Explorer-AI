...
* **Experiment 11 (the rod's own axes):** approved specification (`11-rods-own-axes.md`), physics model (`lorentzTransform` in `src/physics/lorentzTransform.ts`, plus `rodTimeAxisFor`/`rodSpaceAxisFor` in `src/physics/spacetimeDiagramView.ts`), interface, prediction, results panel, and tutor are built, and it has had its complete-flow test in a browser and its final review against its specification (§21 Step 10; no gaps found). It introduces the Lorentz transformation, reusing Experiment 3's time dilation factor by division (via the new `timeDilationFactorFor` helper) rather than a new symbol, and draws the rod's own two axes on the Experiment 8/9 diagram. Applying the transformation to Experiment 7's two events confirms, by direct calculation, that they are simultaneous in the rod's frame. The owner's line-by-line wording approval (§28.1) has not yet been done. Per its specification, this completes "Relativity of Time and Motion." The chapter summary in `src/App.tsx` points to no further experiment, because none is approved yet.
...
**Experiment 11 is complete: built, complete-flow tested, and reviewed against its specification (§21 Step 10). Its line-by-line wording approval (§28.1) has not yet been done — the learner-facing text is a first draft; this is the one item the owner has deferred. With Experiment 11 done, "Relativity of Time and Motion" is complete, per its own specification. Next: Stage 1 of §23, to propose the first experiment of Phase 2 ("Gravity and Curved Spacetime").**
