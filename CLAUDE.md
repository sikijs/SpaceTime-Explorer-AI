...
* **Experiment 7 (relativity of simultaneity):** approved specification (`07-relativity-of-simultaneity.md`), tested physics model (`src/physics/relativityOfSimultaneityExperiment.ts`), interface, prediction, results panel, and tutor are built, and it has had its complete-flow test in a browser. It reuses the Experiment 6 rod and shows that a flash released from its center reaches both ends at the same time in the rod's own frame but not in the lab frame, reusing Experiment 5's invariant `c` and Experiment 6's length contraction without re-deriving them. It has not yet had a final review against its specification (§21 Step 10) or the owner's line-by-line wording approval (§28.1); the chapter summary in `src/App.tsx` points to no further experiment, because none is approved yet.
...
**Experiment 7 is built and complete-flow tested. Remaining: a final review against its specification (§21 Step 10), the owner's line-by-line wording approval (§28.1), and then Stage 1 of §23 to propose the next experiment's direction.**
